...
def main():
    input_path = input("请输入游戏data文件夹的路径（拖动进来即可）: ")
    actors_file = os.path.join(input_path, 'Actors.json')

    actors_data = read_json_file(actors_file)

    # 过滤掉为空的角色名
    character_names = [actor['name'] for actor in actors_data if actor and actor.get('name') and actor['name'].strip()]
    ...
    selected_index = int(input("请输入要修改的角色的编号: ")) - 1
    selected_character = sorted_characters[selected_index][0]
    ...
    if selected_character:
        ...
        for file in os.listdir(input_path):
            if file.lower().endswith('.json'):
                ...
                replacements = find_and_replace_in_object(json_data, selected_character, new_name)
